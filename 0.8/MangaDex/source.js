...
var _Sources = (() => {
  ...
  async function parseAccessToken(accessToken) {
    ...
    const tokenBodyJSON = import_buffer.Buffer.from(tokenBodyBase64, "base64").toString("ascii");
            ...
            App.createDUISection({
              ...
              rows: async () => [
                App.createDUIButton({
                  ...
                  label: "Refresh Token",
                  onTap: async () => {
                    const response = await authEndpointRequest(requestManager, "refresh", { token: accessToken2.refreshToken });
